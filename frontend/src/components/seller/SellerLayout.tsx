...
function SellerLayout({ children, title, subtitle }: SellerLayoutProps) {
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      >
        ...
        <div className="flex h-16 sm:h-20 items-center justify-between border-b border-slate-100 px-5 sm:px-6">
          <Link to="/seller" className="flex items-center gap-3">
            <div className="flex h-9 w-9 sm:h-10 sm:w-10 items-center justify-center rounded-xl bg-indigo-600 text-base sm:text-lg font-bold text-white shadow-sm">
              <Store size={20} />
            </div>
            <div>
        ...
        <header className="sticky top-0 z-30 flex h-16 sm:h-20 items-center justify-between border-b border-slate-200 bg-white/95 px-4 sm:px-8 backdrop-blur">
          ...
          <div className="flex items-center gap-2 sm:gap-3 shrink-0">
            ...
            <div className="flex items-center gap-2 sm:gap-3 rounded-2xl border border-slate-200 bg-slate-50/80 p-1.5 sm:p-2 sm:pr-4">
              <div className="flex h-7 w-7 sm:h-8 sm:w-8 items-center justify-center rounded-xl bg-indigo-600 text-xs font-bold text-white shadow-sm">
                S
              </div>
              <div className="hidden sm:block text-left">
                <span className="block text-xs font-bold text-slate-900 leading-tight">Seller</span>
                <span className="text-[10px] text-slate-400">Penjual Terdaftar</span>
              </div>
            </div>
          </div>
        </header>
      ...
      {/* SELLER LOGOUT CONFIRMATION MODAL */}
      <ConfirmModal
        isOpen={logoutModalOpen}
        title="Konfirmasi Logout Seller"
        message="Apakah Anda yakin ingin keluar dari akun Seller Center? Anda harus masuk kembali untuk mengelola produk dan pesanan toko."
